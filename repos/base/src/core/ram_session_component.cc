...
int Ram_session_component::_transfer_quota(Ram_session_component *dst, size_t amount)
{
	...
	if(strcmp(_label, "")!=0) {
		//printf("I am %s, I have %d, I use %d\n", _label, _quota_limit, _payload);
		_trace_sources.set_quota(_quota_limit, _label);
	} else {
		//PDBG("empty label quota:%d used%d", _quota_limit, _payload);
		
	}
	if(strcmp(_label, "i")!=0) {
		//PDBG("init changed");
	}
...
Ram_dataspace_capability Ram_session_component::alloc(size_t ds_size, Cache_attribute cached)
{
	...
	if(strcmp(_label, "")!=0) {
		...
		//PDBG("empty label quota:%d used%d", _quota_limit, _payload);
		//_trace_sources.set_quota(_quota_limit, "init");
	}
	if(strcmp(_label, "i")!=0) {
		//PDBG("init changed");
	}
